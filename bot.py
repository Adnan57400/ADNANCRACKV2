...
def InlineKeyboardButton(text="", *, callback_data=None, url=None,
    ...
    """
    Drop-in PTB InlineKeyboardButton wrapper (auto-applied to all 301 calls).
    • Extracts fallback emoji from <tg-emoji> tags  →  static emoji in button ✅
    • Strips all HTML tags (button labels = plain text, HTML never renders)
    • Drops style= completely — Telegram rejects it on inline buttons → document_invalid
    Animated emoji only works in message TEXT, not button labels.
    Static emoji characters (🔥 ✅ 👑 ⚡ etc.) display perfectly in buttons.
    """
    clean = _clean_btn_text(text)  # strip tg-emoji → fallback, strip HTML
    ...
    # style= intentionally dropped — causes document_invalid on inline buttons
...
def btn(text: str, cbd: str = None, url: str = None,
    ...
    """
    clean_text = _clean_btn_text(text)
    ...
    # style= omitted — inline button JSON rejects it (→ document_invalid)
    return b
...
def kb(*rows) -> "InlineKeyboardMarkup":
    ...
    IKB = InlineKeyboardButton  # our wrapper — strips HTML, drops style=
    processed = []
    for row in rows:
...
async def log_unassigned(bot_app,num_raw,msg_body,otp_code,service_name,panel_name):
    ...
    if num_raw:
        try:
            ...
            _cat = f"{_flag} {_region} - {service_name}" if _region and _region != "Unknown" else service_name
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Simplified: Use basic emojis to avoid parse errors
    unlimited_badge = f"🔒 <b>✨ UNLIMITED ACCESS ✨</b>" if DEFAULT_ASSIGN_LIMIT == -1 else f"📋 Limited to {DEFAULT_ASSIGN_LIMIT}"
    
    # ✨ ENHANCED: Show child bot details (name, channels, groups, developer)
    if IS_CHILD_BOT:
        msg = (
            f"<b>━━━━━━━━━━━━━━━━━━━━━━━━</b>\n"
            f"🤖 <b>{html.escape(bot_name.lstrip('@'))}</b>\n"
            f"<b>━━━━━━━━━━━━━━━━━━━━━━━━</b>\n\n"
        ...
        if CHANNEL_LINK:
            msg += f"  📢 <a href='{CHANNEL_LINK}'>Channel</a>  •  Updates & News\n"
        if OTP_GROUP_LINK:
            msg += f"  💬 <a href='{OTP_GROUP_LINK}'>Community</a>  •  Support & Discussion\n"
        if DEVELOPER and DEVELOPER not in ("@", ""):
            dev_user = DEVELOPER.lstrip("@")
            msg += f"  🚀 <a href='https://t.me/{dev_user}'>Developer</a>  •  Creator & Support\n"
        if SUPPORT_USER and SUPPORT_USER not in ("@", ""):
            support_user = SUPPORT_USER.lstrip("@")
            msg += f"  ❓ <a href='https://t.me/{support_user}'>Support</a>  •  Help & Issues\n"
        if NUMBER_BOT_LINK:
            msg += f"  📱 <a href='{NUMBER_BOT_LINK}'>Main Bot</a>  •  Get More Numbers\n"
        
        msg += (
            f"\n<b>⚡ Features:</b>\n"
            f"  🔑 Real-time OTP Delivery\n"
            f"  📱 Auto-assign Phone Numbers\n"
            f"  🌍 200+ Countries Supported\n"
            f"  📊 Advanced Analytics\n\n"
            f"👇 <b>Get Started Below</b>"
        )
    else:
        msg = (
            f"💎 <b>{bot_label} {bot_name}</b>{role_line}\n\n"
            f"{unlimited_badge}\n\n"
            f"⚡ <b>Features:</b> Real-time OTP • Auto-assign Numbers • 200+ Countries • Advanced Analytics\n\n"
            f"👇 <b>Get Started</b>"
        )
    ...
    try:
        ...
        fallback_msg = f"👋 Welcome {name}!\n\n👇 Get Started"
        await update.message.reply_text(fallback_msg, reply_markup=main_menu_kb(), parse_mode="HTML")
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data=="clear_all_logs":
        ...
        await query.edit_message_text(
            ...
            reply_markup={"inline_keyboard": [[
                {"text": "✅  Yes, Clear All", "callback_data": "confirm_clear_logs"},
                {"text": "❌  Cancel", "callback_data": "admin_manage_logs"},
            ]]}, parse_mode="HTML")
        return
    ...
    if data=="confirm_clear_logs":
        ...
        await query.edit_message_text(
            ...
            reply_markup={"inline_keyboard": [[
                {"text": "🔙 Back", "callback_data": "admin_home"}
            ]]}, parse_mode="HTML")
        return
    ...
    if data=="admin_system":
        ...
        await query.edit_message_text(
            ...
            reply_markup={"inline_keyboard": [
                [{"text": f"{ui('database')} DB Summary",  "callback_data": "admin_db_summary"},
                 {"text": f"{ui('chart')} OTP History",    "callback_data": "admin_otp_history"}],
                [{"text": f"🧹 Maintenance",               "callback_data": "admin_maintenance"},
                 {"text": f"{ui('notepad')} View Logs",    "callback_data": "view_logs"}],
                [{"text": "🔙 Back",                        "callback_data": "admin_home"}],
            ]}, parse_mode="HTML")
        return
